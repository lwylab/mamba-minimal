...
# - 多任务学习 ：同时进行多分类（预测具体故障类型）和二分类（预测是否有故障）任务，提高模型对少数类的识别能力。
# - 专用分类器 ：为TWF和RNF这两个难以识别的故障类型添加了专用的二分类器，提高它们的识别准确率。
# - 焦点损失 ：使用焦点损失函数，对难以分类的样本给予更高的权重，提高模型对少数类的学习能力。
# - 类别权重 ：对TWF和RNF类别使用更高的权重，使模型更加关注这些类别。
# - 高级特征工程 ：创建了多种工程特征，特别是针对TWF和RNF故障类型的特征组合。
# - Mixup数据增强 ：使用Mixup技术进行数据增强，提高模型的泛化能力。
# - 加权采样 ：使用加权随机采样器，确保每个批次中都有足够的少数类样本。
# - 硬样本挖掘 ：关注那些难以分类的样本，提高模型的学习效果。
...
class EnhancedSequenceModel(nn.Module):
    def __init__(self, d_model, d_state=16, expand=2, dropout=0.1):
        ...
        logging.info(f"初始化增强版序列模型，d_model={d_model}, d_state={d_state}")
...
class EnhancedConfig:
    ...
    model_save_path: str = "best_enhanced_model.pth"
    base_output_dir: str = "enhanced_experiment_results"  # 基础输出目录
    experiment_data_path: str = "enhanced_evaluation_results.json"
    batch_size: int = 32  # 减小批量大小，提高模型对少数类的敏感度
    num_epochs: int = 300  # 增加训练轮次
    early_stopping_patience: int = 50  # 增加早停耐心值

    # 模型参数
    d_model: int = 512  # 增加模型维度
    n_layer: int = 6  # 增加层数
    expand: int = 4
    ...
    # 硬样本挖掘参数
    use_hard_mining: bool = True  # 使用硬样本挖掘
    hard_mining_ratio: float = 0.3  # 硬样本比例
...
class EnhancedFaultPredictionModel(nn.Module):
    """增强的故障预测模型"""
    
    def __init__(self, config: EnhancedConfig, input_dim: int, num_classes: int):
        super().__init__()
        self.config = config
...
class EnhancedModelTrainer:
    ...
    def __init__(self, config: EnhancedConfig, model: EnhancedFaultPredictionModel):
        self.config = config
        self.model = model
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            ...
            patience=10,
            min_lr=1e-6,
            verbose=False  # 明确设置verbose为False，避免警告
        )
        ...
        if config.use_focal_loss:
            self.criterion_multi = FocalLoss(
                ...
                weight=self.config.class_weights if config.use_class_weights else None
            )
        else:
...
class EnhancedModelEvaluator:
    ...
    def evaluate(self, model, X_test, y_test, fault_mapping):
        ...
        plt.xlabel('召回率')
        plt.ylabel('精确率')
        plt.title('多分类精确率-召回率曲线')
        plt.legend(loc="lower left")
        plt.tight_layout()
        plt.savefig(os.path.join(self.config.output_dir, "precision_recall_curve.svg"), format='svg', bbox_inches='tight')
        plt.close()
        
        # 绘制二分类精确率-召回率曲线
        plt.figure(figsize=(8, 6))
        precision, recall, thresholds = precision_recall_curve(y_test_binary, y_prob_binary)
        avg_precision = average_precision_score(y_test_binary, y_prob_binary)
        ...
        plt.legend(loc="lower left")
        plt.tight_layout()
        plt.savefig(os.path.join(self.config.output_dir, "binary_precision_recall_curve.svg"), format='svg', bbox_inches='tight')
        plt.close()
        ...
        for threshold in thresholds:
            y_pred_binary_threshold = (y_prob_binary >= threshold).astype(int)
            f1 = f1_score(y_test_binary, y_pred_binary_threshold, zero_division=0)
            f1_scores.append(f1)
        ...
        # 使用最佳阈值的预测结果
        y_pred_binary_best = (y_prob_binary >= best_threshold).astype(int)
        best_precision = precision_score(y_test_binary, y_pred_binary_best, zero_division=0)
        best_recall = recall_score(y_test_binary, y_pred_binary_best, zero_division=0)
        
        logging.info(f"最佳阈值: {best_threshold:.4f}")
        logging.info(f"最佳阈值下的F1分数: {best_f1:.4f}")
        logging.info(f"最佳阈值下的精确率: {best_precision:.4f}")
        logging.info(f"最佳阈值下的召回率: {best_recall:.4f}")
        
        # 为每种故障类型计算性能指标
        for i, fault_type in enumerate(fault_mapping.values()):
            if i == 0:  # 跳过无故障类别
                continue
            
            # 计算当前故障类型的性能
            y_test_fault = (y_test == i).astype(int)
            y_pred_fault = (y_pred == i).astype(int)
            
            fault_precision = precision_score(y_test_fault, y_pred_fault, zero_division=0)
            fault_recall = recall_score(y_test_fault, y_pred_fault, zero_division=0)
            fault_f1 = f1_score(y_test_fault, y_pred_fault, zero_division=0)
            fault_support = np.sum(y_test_fault)
            ...
            # 添加到评估结果
            evaluation_results[f'{fault_type}_metrics'] = {
                'precision': float(fault_precision),
                'recall': float(fault_recall),
                'f1': float(fault_f1),
                'support': int(fault_support)
            }
...
def main():
    ...
    # 初始化模型
    model = EnhancedFaultPredictionModel(
        config=config,
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(config.output_dir, "loss_curve.svg"), format='svg', bbox_inches='tight')
    plt.close()
    
    # 初始化评估器
    evaluator = EnhancedModelEvaluator(config)
