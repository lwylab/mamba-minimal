...
class Config:
    ...
    base_output_dir: str = "experiment_results"  # 基础输出目录
    experiment_data_path: str = "evaluation_results.json"
    batch_size: int = 128
    num_epochs: int = 100
    ...
    num_layers: int = 2
    dropout: float = 0.3
    dim_feedforward: int = 512  # 前馈网络维度，通常是d_model的4倍
...
class FaultTypeTransformer(nn.Module):
    """用于故障类型预测的Transformer模型"""
    ...
    def __init__(self, config: Config, input_dim: int, num_classes: int):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(config.dim_feedforward, num_classes)
        )
...
class ModelEvaluator:
    ...
    def evaluate(self, model: nn.Module, X_test_tensor: torch.Tensor, y_test: np.ndarray, fault_mapping: Dict):
        ...
        rects1 = ax.bar(x - width/2, precision, width, label='精确率')
        rects2 = ax.bar(x + width/2, recall, width, label='召回率')
...
def main():
    ...
    # 初始化模型
    model = FaultTypeTransformer(config, input_dim=input_dim, num_classes=config.num_classes).to(config.device)
    ...
    logging.info(f"最重要的5个特征: {', '.join(top_features)}")
    
    return model, evaluation_results, feature_importance
