"""
基于Mamba2状态空间模型的故障预测系统
使用Mamba2架构进行时序数据建模和故障预测
"""
...
# 导入简化版Mamba2模型组件
from mamba2_simplified import Mamba2Config, Mamba2, ResidualBlock, RMSNorm
...
class Mamba2SequenceModel(nn.Module):
    """基于Mamba2的序列模型"""
    def __init__(self, input_dim: int, num_classes: int, config: Mamba2FaultConfig):
        super().__init__()
        self.config = config
        
        # 创建Mamba2模型参数
        self.mamba2_config = Mamba2Config(
        ...
        # Mamba2模型
        self.mamba2 = Mamba2(self.mamba2_config)
...
class DataProcessor:
    """数据处理类"""
    def __init__(self, config: Mamba2FaultConfig):
        self.config = config
...
class ModelEvaluator:
    ...
    def _plot_roc_curve(self, y_true, y_pred_proba, fault_mapping):
        ...
        # 将真实标签转换为one-hot编码
        y_true_onehot = np.zeros((len(y_true), len(fault_mapping)))
        ...
        for i in range(len(fault_mapping)):
            ...
            roc_auc = auc(fpr, tpr)
            plt.plot(fpr, tpr, lw=2, label=f'{fault_mapping[i]} (AUC = {roc_auc:.2f})')
    ...
    def _plot_precision_recall_curve(self, y_true, y_pred_proba, fault_mapping):
        ...
        for i in range(len(fault_mapping)):
            ...
            plt.plot(recall, precision, lw=2, label=f'{fault_mapping[i]} (AP = {ap:.2f})')
...
def main():
    ...
    # 创建模型
    input_dim = X_test.shape[2]  # 特征维度
    num_classes = len(fault_mapping)  # 类别数
    model = Mamba2SequenceModel(input_dim, num_classes, config).to(config.device)
    ...
    model = trainer.train(model, train_loader, X_test, y_test, fault_mapping, data_processor.class_weights)
    
    # 评估模型
    evaluator = ModelEvaluator(config)
    results = evaluator.evaluate(model, X_test, y_test, fault_mapping)
    ...
    logging.info(f"评估结果已保存到: {os.path.join(config.output_dir, 'evaluation_results.json')}")
